...
from edge_detection import Plot
...
def canny(image, sigma=1., low_threshold=None, high_threshold=None, mask=None,
    ...
    * Perform a hysteresis thresholding: first label all points above the
    ...
    magnitude = np.hypot(isobel, jsobel)#  Given the “legs” of a right triangle, return its hypotenuse.
...
if __name__ == "__main__":
    #plot = Plot("test canny")
    img = cv2.imread(config.img_path, 0)
    img = img.astype(np.float)
    sigma = [0, 0.1, 1, 5, 10]
    for i in sigma:
        img_c = canny(img, i,0, 255)
    #plot.append(img, "default")
    #plot.append(img_c, "canny")
    #plot.show()
